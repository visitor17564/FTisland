...
// 댓글
const target_type = "댓글";

// 내가 좋아하는 댓글 조회API(수정필요)
router.get("/user/my_liked_comments", authMiddleware, async (req, res) => {
...
// Like버튼 누르기
router.post("/posts/:postId/comments/:commentId/likes", authMiddleware, async (req, res) => {
  // 구조분해할당
  const { userId } = req.user;
  const { postId } = req.params;
...
// Likes 취소버튼 누르기
router.delete("/posts/:postId/comments/:commentId/likes", authMiddleware, async (req, res) => {
  // 구조분해할당
  const { userId } = req.user;
  const { postId } = req.params;
